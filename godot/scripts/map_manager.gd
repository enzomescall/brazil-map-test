...
func _register_province_from_json(entry: Dictionary) -> void:
	# --- Required fields ---
	var province_name: String = entry.get("name", "")
	var color_html: String = entry.get("color_html", "")

	if province_name == "" or color_html == "":
	...
	if color == null:
		push_warning("Invalid color hex '%s' for province %s" % [color_html, province_name])
		return
	...
	register_province(province_name, color, population, gdp_billions)
...
func handle_click(global_pos: Vector2) -> void:
	...
	if province_registry.has(color_key):
		...
		print("Selected Province: " + selected_province.province_name)
